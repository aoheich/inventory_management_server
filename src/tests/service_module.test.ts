...
describe("tests the login_user service", () => {
    ...
    it("throws an error if a wrong password is entered", async () => {
        ...
        vi.mocked(bcrypt.compare).mockResolvedValue()// false or empty (true here doesn't work truthy value i think)
...
describe("tests the transaction_service", () => {
    ...
    it("tests a failed transaction creation", async () => {
       ...
       const data = {
        ...
        await expect(create_transaction(data, 1)).rejects.toThrowErrorMatchingInlineSnapshot(`[Error: Database Error: Transaction Failed]`)
        // testing the snapshot code out, his could have just been toThrow("Database Error: Transaction Failed")
